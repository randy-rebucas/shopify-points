...
(function(){
    ...
    var myAppJavaScript = function($){
        ...
        if(urlParams.has('tpsession_id')) {
            ...
            $('head').append($('<link>').attr('href','https://98f35fa5.ngrok.io/stylesheets/style.css'));
            let pointWidgetCount = 0;
            let pointWidgetTitle = 'Your Tasty Point';
            const pointEl = `<div style="position: fixed;
            right: 0;
            bottom: 30%;
            background: red;
            ...
            box-shadow: -7px 7px 9px -8px rgba(0,0,0,0.73);"><h5 style="padding: 0;
            margin: 0;
            text-transform: uppercase;
            text-align: right;">${pointWidgetTitle}</h5><p style="font-size: 28px;
            color: #fff;
            text-align: right;
            line-height: .8;">${pointWidgetCount}</p></div>`;
            $('body').append(pointEl);
