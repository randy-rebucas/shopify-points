...
router.get('/theme', (req, res, next) => {
  ...
  request.get(shopRequestUrl, { headers: shopRequestHeaders })
  .then((shopResponse) => {
    res.render('theme', { title: 'Theme', data: shopResponse })
    // res.status(200).end(shopResponse);
  })
  .catch((error) => {
    res.status(error.statusCode).send(error.error.error_description);
...
router.post('/assets', (req, res, next) => {
  ...
  //     }
  // }
  // req.shopifyToken.put('/admin/api/2020-01/themes/'+req.themeId+'/assets.json', assetConfig, function(err, configData, headers) {
  //     res.redirect('/');
  // });
});
