...
router.get('/', (req, res) => {
    ...
    .then((metafiledsResponse) => {
        ...
        const val = JSON.parse(JSON.stringify(metafieldObjs.value));
        // JSON.parse(JSON.stringify(metaFormVal));
        console.log(val);
        console.log(metafieldObjs.value);
        res.render('points', { title: 'Metafields', metafieldObj: metafieldObjs});
    })
    .catch((error) => {
...
router.put('/', (req, res) => {
    ...
    const metaFormVal = {
        ...
        isRounded: req.body.isRounded
    }
    const metaValue = JSON.parse(JSON.stringify(metaFormVal));

    console.log(metaValue);

    // const metafieldsRequestUrl = 'https://' + shopCookie.shopOrigin + '/admin/api/2020-01/metafields/' + req.body.id + '.json';
